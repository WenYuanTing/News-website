...
const Sports = ({ input, setInput }) => {
  ...
  const initailUrl =
    `https://newsapi.org/v2/everything?q=sports&page=1&pageSize=15&apiKey=` +
    auth;
  const searchUrl =
  ...
  const moreNews = async () => {
    ...
    if (currentsearch === "") {
      newUrl =
        `https://newsapi.org/v2/everything?q=sports&page=${
          page + 1
  ...
  return (
    <div style={{ minHeight: "100vh" }}>
      <Search
        ...
        setInput={setInput}
      />
      <h2 style={{ margin: "0rem 3rem" }}>精選新聞 - 運動版面</h2>
      <div className="news">
        {data &&
          data.map((d) => {
            return <News data={d} />;
