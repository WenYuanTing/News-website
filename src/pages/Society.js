...
const Society = ({ input, setInput }) => {
  ...
  const initailUrl =
    `https://newsapi.org/v2/everything?q=society&page=1&pageSize=15&apiKey=` +
    auth;
  const searchUrl =
  ...
  const moreNews = async () => {
    ...
    if (currentsearch == null) {
      newUrl =
        `https://newsapi.org/v2/everything?q=society&page=${
          page + 1
  ...
  return (
    <div style={{ minHeight: "100vh" }}>
      <Search
        ...
        setInput={setInput}
      />
      <h2 style={{ margin: "0rem 3rem" }}>精選新聞 - 社會版面</h2>
      <div className="news">
        {data &&
          data.map((d) => {
            return <News data={d} />;
