...
const Worldwide = ({ input, setInput }) => {
  ...
  const initailUrl =
    `https://newsapi.org/v2/everything?q=world&page=1&pageSize=15&apiKey=` +
    auth;
  const searchUrl =
  ...
  const moreNews = async () => {
    ...
    if (currentsearch == null) {
      console.log("進入到未搜尋的更多頁面");
      newUrl =
        `https://newsapi.org/v2/everything?q=world&page=${
          page + 1
  ...
  return (
    <div style={{ minHeight: "100vh" }}>
      <Search
        ...
        setInput={setInput}
      />
      <h2 style={{ margin: "0rem 3rem" }}>精選新聞 - 全球版面</h2>
      <div className="news">
        {data &&
          data.map((d) => {
            return <News data={d} />;
